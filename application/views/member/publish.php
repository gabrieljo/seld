	...
	<div class="row">
		<div class="col-sm-7 nopadding">
			...
			<dl class="dl-horizontal">
				...
				<dt>Description</dt>
				<dd><?=word_limiter($product->pr_description, 30)?></dd>
			...
			<div class="panel panel-info">
				...
				<div class="panel-body">
					...
					<dl class="dl-horizontal marketInfo">
						...
						<dt>Description</dt>
						<dd><?=word_limiter($product->pr_mk_description, 40)?></dd>

						<dt></dt>
						<dd><hr></dd>
							
						<dt>Total Buyers</dt>
						<dd><?=$summary->totalTrans?></dd>

						<dt>Total Earnings</dt>
						<dd><h4>₩ <strong><?=number_format($summary->totalAmount)?></strong></h4></dd>
					...
					<div class="marketForm hidden">
						<?=form_open('m/publish/' . $product->pr_uid, array('class'=>'form-horizontal', 'id'=>'frmAddMarket', 'data-status'=>$product->pr_mk_status))?>
							...
							<div class="form-group">
								<label for="mk_price" class="col-sm-4 control-label">Category</label>
								<div class="col-sm-4">
									<div class="form-group">
										<div class="input-group">
											<?php
											$options 	= array();
											$parent 	= $category == null ? 0 : $category->cat_parent;
											foreach ($categories->result() as $item){
												if ($item->cat_parent != 0) continue;
												$options['' . $item->cat_id] = $item->cat_name;
											}
											echo form_dropdown('pr_category', $options, $parent, 'class="form-control" id="pr_category"');
											?>
										</div>
									</div>
								</div>
							...
							<div class="form-group">
								<label for="mk_price" class="col-sm-4 control-label">Sub-Category</label>
								<div class="col-sm-4">
									<div class="form-group">
										<div class="input-group">
											<?php
											$options = array();
											echo form_dropdown('pr_cat', $options, '', 'class="form-control" id="pr_cat"');
											?>
										</div>
									</div>
								</div>
...
$(function(){
	...
	$('#pr_category').change(function(){
		var vl = $('#pr_category').val();
		getSubCategories(vl);
	});

	getSubCategories(<?=($category == null) ? 0 : $category->cat_parent?>);
});

function getSubCategories(vl){
	$.ajax({
		type: 'get',
		data: '',
		url: base_url() + 'm/get_subs/' + vl + '/<?=$product->pr_cat_id?>',
